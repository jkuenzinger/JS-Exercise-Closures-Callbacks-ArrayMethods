...
function processSum(numberList, callback) {
  return callback (numberList.reduce((accumulator, num)=> accumulator + num
, 0 ));
}
...
function processContains(item, list, callback) {
    const newList = (callback(list.includes(item)));
    return newList;
}
...
function tallyUpDonations(runners) {
  return runners.reduce((accumulator, runner) => accumulator + runner.donation
, 0)
}
